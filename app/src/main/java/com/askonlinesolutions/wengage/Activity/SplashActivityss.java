            ...
            if (bundle != null) {
                try {
                    if (bundle.containsKey("data")) {
                        ...
                        Log.d("MyFireBasePayLoad", object.toString());
                        if(object.has("type")) {
                            notification_type = object.optInt("type");
                        }
                        if(object.has("status")) {
                            status = object.optInt("status");
                            status1 = object.optString("status");
                        }
                        if(object.has("notification_id")){
                            notification_id = object.optString("notification_id");
                        }

                        Log.e("notification_type", notification_type + "");
                        Log.e("status", status + "");
                        Log.d("MyStatus", status1 + "");
                    }
            ...
            if (latitude != 0 && longitude != 0) {
                new Handler().postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            String s = getAddress(new LatLng(latitude,longitude)).toString();
                            tinyDB.putString("chef_country_code", s);    // For Chef Home
                            tinyDB.putString("foodie_country_code", s);  // For Foodie Home
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
